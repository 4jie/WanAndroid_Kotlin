...
import com.caisijie.common.constant.TABLE_VIDEO_LIST
import com.caisijie.room.entity.VideoInfo

/**
 * @author caisijie
 * @date 2023/9/18 23:42
 * @desc 定义数据操作的方法，增删改查一般为interface
 * 注意：下面的函数需要确保正确，否则会出现不可预知问题
 */
@Dao
interface VideoListCacheDao {
    ...
    /**
     * 删除指定item
     * 使用主键将传递的实体实例与数据路中的行进行匹配
     * 如果没有具有相同主键的行，则不会进行任何更改
     */
    @Delete
    ...
    /**
     * 更新某个item
     * 不指定的entity也可以，会根据你传入的参数对象来找你要操作的那张表
     */
    @Update
