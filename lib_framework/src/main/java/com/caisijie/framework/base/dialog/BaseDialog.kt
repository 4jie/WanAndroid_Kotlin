...
//            }
            ...
            //将像素值转换为浮点数的目的是为了满足 WindowManager.LayoutParams 对象的要求。
            // 该对象中的边距属性（horizontalMargin 和 verticalMargin）接受的是浮点数值，表示边距占整个对话框宽度或高度的比例。
            //例如，假设 mHorizontalMargin 的值为 20 像素，对话框的宽度为 200 像素。
            //将 mHorizontalMargin 转换为浮点数后，可以得到 0.1，表示水平边距占对话框宽度的 10%。
            //这种比例表示的方式可以使得对话框在不同屏幕尺寸和分辨率上具有一致的外观和布局，而不依赖于固定的像素值。
