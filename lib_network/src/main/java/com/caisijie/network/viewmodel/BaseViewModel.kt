...
import com.caisijie.network.callback.IApiErrorCallback
...
open class BaseViewModel : ViewModel() {
    ...
    private suspend fun <T> safeApiCall(
        ...
        try {
            ...
            errorBlock(exception.errorCode, exception.errMsg)
        }
        return null
    }
    ...
    fun <T> launchUIWithResult(
        ...
        viewModelScope.launch(Dispatchers.Main) {
            val result = safeApiCallWithResult(errorCall, responseBlock)
            successBlock(result)
        }
    }
    ...
    private suspend fun <T> safeApiCallWithResult(
        ...
        try {
            ...
            LogUtils.e(e)
            val exception = ExceptionHandler.handleException(e)
            if (ERROR.UN_LOGIN.code == exception.errorCode) {
                errorCall?.onLoginFail(exception.errorCode, exception.errMsg)
            } else {
                errorCall?.onError(exception.errorCode, exception.errMsg)
            }
        }
        return null
    }
