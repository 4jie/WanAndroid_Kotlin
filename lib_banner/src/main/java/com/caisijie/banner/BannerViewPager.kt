...
import com.caisijie.banner.annotation.AIndicatorGravity
import com.caisijie.banner.annotation.AIndicatorSlideMode
import com.caisijie.banner.annotation.AIndicatorStyle
import com.caisijie.banner.annotation.APageStyle
import com.caisijie.banner.annotation.Visibility
import com.caisijie.banner.base.BaseBannerAdapter
import com.caisijie.banner.base.BaseBannerAdapter.Companion.MAX_VALUE
import com.caisijie.banner.base.BaseViewHolder
import com.caisijie.banner.indicator.IIndicator
import com.caisijie.banner.indicator.IndicatorView
import com.caisijie.banner.manager.BannerManager
import com.caisijie.banner.manager.ReflectLayoutManager
import com.caisijie.banner.mode.PageStyle
import com.caisijie.banner.options.BannerOptions
import com.caisijie.banner.options.BannerOptions.Companion.DEFAULT_REVEAL_WIDTH
import com.caisijie.banner.options.IndicatorOptions
import com.caisijie.banner.transform.ScaleInTransformer.Companion.DEFAULT_MIN_SCALE
import com.caisijie.banner.utils.BannerUtils
import com.caisijie.banner.utils.BannerUtils.getOriginalPosition
import com.caisijie.framework.utils.ViewUtils
...
open class BannerViewPager<T, H : BaseViewHolder<T>> @JvmOverloads constructor(
    ...
    /**
     * RectF 为一个矩形保存四个浮点坐标。矩形由其4条边（左、上、右、下）的坐标表示。
     * 可以直接访问这些字段。使用 width（） 和 height（） 检索矩形的宽度和高度。
     */
    ...
    /**
     * 重写拦截触摸事件的方法，以避免滑动冲突
     * 关键在parent.requestDisallowInterceptTouchEvent(...)，调用该方法请求父容器不做TouchEvent的拦截
     */
    ...
    /**
     * Start loop 这里设置的的start是自动轮播的start，所以手指离开ViewPager2的Item时，
     * 事件拦截ACTION_UP的话需要重新startLoop()开始自动轮播
     */
    ...
    /**
     * Stop loop 同Start Loop，这里的stop也是停止自动轮播，所以当用户触摸到当前ViewPager2时
     * 也就是ACTION_DOWN时，需要调用stopLoop()来停止自动轮播
     */
    ...
    private fun setupViewPager(list: List<T>, isInitCurrent: Boolean = true) {
        ...
        if (bannerOptions.getScrollDuration() != 0) {
            mViewPager?.let {
                ReflectLayoutManager.reflectLayoutManager(
                    it,
                    bannerOptions.getScrollDuration()
                )
            }
        }
        if (isInitCurrent) {
        ...
        mViewPager?.offscreenPageLimit =
            if (limit > 0) limit else ViewPager2.OFFSCREEN_PAGE_LIMIT_DEFAULT
    ...
    private fun initPageStyle(@APageStyle pageStyle: Int) {
        val pageScale: Float = mBannerManager.getBannerOptions().getPageScale()
    ...
    /**
     * Set indicator dash width，if indicator style is [com.caisijie.banner.mode.IndicatorStyle.CIRCLE],
     * the indicator circle radius is indicatorWidth/2.
     *
     * @param normalWidth if the indicator style is [com.caisijie.banner.mode.IndicatorStyle.DASH]
     * the params means unchecked dash width
     * if the indicator style is [com.caisijie.banner.mode.IndicatorStyle.ROUND_RECT]  means
     * unchecked round rectangle width
     * if the indicator style is [com.caisijie.banner.mode.IndicatorStyle.CIRCLE] means
     * unchecked circle diameter
     * @param checkWidth if the indicator style is [com.caisijie.banner.mode.IndicatorStyle.DASH]
     * the params means checked dash width
     * if the indicator style is [com.caisijie.banner.mode.IndicatorStyle.ROUND_RECT] the
    ...
    fun refreshData(list: List<T>?) {
        post {
            if (isAttachedToWindow && (list != null) && (mBannerPagerAdapter != null)) {
                ...
                mViewPager?.offscreenPageLimit =
                    if (limitSize > 0) limitSize else ViewPager2.OFFSCREEN_PAGE_LIMIT_DEFAULT
